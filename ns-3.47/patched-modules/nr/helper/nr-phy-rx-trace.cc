                                         ...
                                         Ptr<const NrControlMessage> msg)
{
    ...
    uint16_t actualRnti = rnti; 

    if (msg->GetMessageType() == NrControlMessage::DL_DCI)
    {
        NrDlDciMessage* dciMsg = dynamic_cast<NrDlDciMessage*>(const_cast<NrControlMessage*>(PeekPointer(msg)));
        if (dciMsg)
        {
            actualRnti = dciMsg->GetDciInfoElement()->m_rnti;
        }
    }
    else if (msg->GetMessageType() == NrControlMessage::UL_DCI)
    {
        NrUlDciMessage* dciMsg =dynamic_cast<NrUlDciMessage*>(const_cast<NrControlMessage*>(PeekPointer(msg)));
        if (dciMsg)
        {
            actualRnti = dciMsg->GetDciInfoElement()->m_rnti;
        }
    ...
    m_txedGnbPhyCtrlMsgsFile << Simulator::Now().GetNanoSeconds() / (double)1e9 << "\t"
                             ...
                             << static_cast<uint32_t>(sfn.GetSubframe()) << "\t"
                             << static_cast<uint32_t>(sfn.GetSlot()) << "\t" << nodeId << "\t"
                             << actualRnti << "\t" << static_cast<uint32_t>(bwpId) << "\t";
